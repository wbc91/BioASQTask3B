...
public class BioTokenFilter extends TokenFilter{

	private static final Logger Log = LoggerFactory.getLogger(BioTokenFilter.class);
	
	// The list of Bio-phrase character strings
	...
	// If true, emit single tokens as well as phrases
	...
	public BioTokenFilter(Version matchVersion, TokenStream input, CharArraySet phraseSet, boolean emitSingleTokens){
		super(input);
		
		//Convert to CharArrayMap by iterating the char[] strings and
		//putting them into the CharArrayMap with Integer of the number
		//of tokens in the map: need this to determine when a phrase math is completed.
		this.phraseMap = convertPhraseSet(phraseSet);
		this.emitSingleTokens = emitSingleTokens;
	}
	
	protected BioTokenFilter(TokenStream input) {
		super(input);	
	}
	
	public void setReplaceWhiteSpaceWith(Character replaceWithspaceWith){
	...
	public boolean incrementToken() throws IOException {
		...
		if(currentSetToCheck == null || currentSetToCheck.size() == 0){
			...
			if(phraseMap.keySet().contains(nextToken,0,nextToken.length)){
				// get the phrase set for this token, add it to currentPhraseToCheck
			...
			if(!emitSingleTokens){
				//current phrase didn't match fully: put the token back
				//into the unusedTokenList
				discardCharTokens(currentPhrase, unusedTokens);
				currentPhrase.setLength(0);
				currentSetToCheck = null;
